...
class VaultSecretProvider(SecretProvider):
    ...
    """
    The signing key for generating Nyl JWT tokens (required when jwt_auth_method="nyl").
    This should be a shared secret between Nyl and Vault. Can also be provided via
    the NYL_VAULT_JWT_SIGNING_KEY environment variable.
    """
    ...
    _nyl_jwt_token: str | None = field(init=False, repr=False, default=None)

    def set_nyl_jwt_token(self, token: str) -> None:
        """Set the Nyl-issued JWT token for authentication."""
        self._nyl_jwt_token = token
    ...
    def _authenticate_with_nyl_jwt(self) -> None:
        ...
        This token is generated by Nyl based on the ArgoCD application context
        and contains claims about the ArgoCD application being deployed, enabling
        multi-tenant secure access to Vault secrets.
        ...
        """
        if not self._nyl_jwt_token:
            raise RuntimeError(
                "Nyl JWT authentication requested but token not set. "
                "The token should be generated and set by Nyl before accessing secrets."
            )
        ...
        try:
            ...
            self._client.auth.jwt.login(role=self.jwt_role, jwt=self._nyl_jwt_token)
